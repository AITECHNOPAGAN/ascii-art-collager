...
export const EditableAsciiLayer = observer(({ layer, parallaxOffset = { x: 0, y: 0 } }: EditableAsciiLayerProps) => {
    ...
    // HTML editing mode
    if (isHtmlEditMode && isActive) {
        const handleHtmlChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
            const htmlContent = e.target.value;
            layerStore.setLatticeFromText(layer.id, htmlContent);
        };

        const handleBlur = () => {
            layerStore.saveCurrentLayer();
            editingStore.setIsHtmlEditMode(false);
        };

        // Convert lattice to HTML with classes
        const htmlContent = latticeToHtmlWithClasses(layer.lattice);

        const { position, offsetX, offsetY, scale, fontSize, zIndex } = layer;
            ...
            <div style={containerStyle}>
                <textarea
                    ...
                    style={{
                        ...
                        resize: 'none',
                        minWidth: `${layer.lattice.width * fontSize}px`,
                        minHeight: `${layer.lattice.height * fontSize}px`,
                    }}
                    rows={layer.lattice.height}
                    cols={layer.lattice.width}
                />
            </div>
        );
    }
    ...
    if (isTextEditMode && isActive) {
        ...
        const handleBlur = () => {
            layerStore.saveCurrentLayer();
            editingStore.setIsTextEditMode(false);
        };

        // Convert lattice to plain text for editing
        const textContent = layer.lattice.cells.map(row =>
